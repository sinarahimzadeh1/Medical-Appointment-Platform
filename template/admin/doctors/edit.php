        ...
        <div class="col-xl-12 col-12">
          <div class="box">
            <div class="box-header d-flex justify-content-between align-items-center">
              <h4 class="box-title">ویرایش دکتر <?= $doctorr['name'] ?></h4>
            ...
            <div class="box-body">
              ...
              <form method="post" action="<?= url('admin/doctor/update/' . $doctorr['id']) ?>" enctype="multipart/form-data">
                <div class="row">

                  <div class="form-group">
                    <label for="image">تصویر پروفایل قبلی : </label>
                    <img src="<?= asset($doctorr['profile']) ?>" style="width: 200px; border-radius: 5px;" class="my-2"
                      alt="">
                  </div>
                  <div class="form-group">
                  ...
                  <div class="col">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="name" name="name" value="<?= $doctorr['name'] ?>"
                        placeholder="اسم دکتر..." required autofocus>
                    </div>
                    <div class="form-group">
                      <label for="title">شماره تلفن :</label>
                      <input type="text" class="form-control" id="number" name="number"
                        value="<?= $doctorr['number'] ?>" placeholder="شماره تلفن..." required autofocus>
                    </div>
                    <div class="form-group">
                      <label for="title">کد نظام پزشکی :</label>
                      <input type="text" class="form-control" id="doc_number" value="<?= $doctorr['doc_number'] ?>"
                        name="doc_number" placeholder="کد نظام پزشکی..." required autofocus>
                    </div>
                    <div class="form-group">
                      <label for="title">سال شروع فعالیت :</label>
                      <input data-jdp type="text" class="form-control" id="experience"
                        value="<?= $doctorr['experience'] ?>" name="experience" placeholder="فقط سال مهم است..."
                        required autofocus>
                    </div>
                  </div>
                  ...
                  <div class="col">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="surgery_phone"
                        value="<?= $doctorr['surgery_phone'] ?>" name="surgery_phone" placeholder="تلفن مطب..." required
                        autofocus>
                    </div>
                    <div class="form-group">
                      <label for="title">کلینیک :</label>
                      <input type="text" class="form-control" id="clinic" name="clinic"
                        value="<?= $doctorr['clinic'] ?>" placeholder="کلینیک..." required autofocus>
                    </div>
                    <div class="form-group">
                      <label for="title">آدرس :</label>
                      <input type="text" class="form-control" id="location" name="location"
                        value="<?= $doctorr['location'] ?>" placeholder="آدرس..." required autofocus>
                    </div>
                    <div class="form-group">
                      <label for="title">شهر :</label>
                      <input type="text" class="form-control" id="city" name="city" value="<?= $doctorr['city'] ?>"
                        placeholder="شهر..." required autofocus>
                    </div>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="expert" name="expert"
                      placeholder="مثال : دندان پزشک، پزشک پوست و مو..." value="<?= $doctorr['expert'] ?>" required
                      autofocus>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <div style="display: flex; align-items: center;">
                      ...
                      <div id="tagContainer">
                        <input type="text" id="tagInput" placeholder="تخصص رو بنویس و + رو بزن، مثلا: لمینت (تخصص تکراری ننویس)">
                      </div>
                    </div>
                    <input type="hidden" name="services" id="hiddenTags">
                    <input type="hidden" id="presetServices" value="<?= $doctorr['services'] ?>">
                  </div>

                  <div class="form-group">
                    <label>انتخاب‌ مسیر در نقشه :</label>
                    <input value="<?= $doctorr['coords'] ?>" type="text" class="form-control my-3" id="coords" name="coords"
...
<script>
  ...
  // ساخت نقشه
  const defaultLat = <?= trim($lat) ?>;
  const defaultLng = <?= trim($lng) ?>;

  // مقدار پیش‌فرض از input
  ...
  // اگر مقدار اولیه وجود داشت، مارکر هم بذار
  if (coordsValue) {
    const [lat, lng] = coordsValue.split(',').map(Number);
    if (!isNaN(lat) && !isNaN(lng)) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const presetInput = document.getElementById('presetServices');
    ...
    function createTag(text) {
      ...
      span.className = 'badge badge-primary mx-1 fs-6 my-1';
      span.textContent = text;

      const remove = document.createElement('span');
      remove.className = 'ml-2 text-white mx-1 fs-6';
    ...
    // مقدار اولیه services (مثل: "ایمپلنت, لمینت")
    const preset = presetInput.value;
    if (preset) {
      preset.split(',').forEach(tag => {
